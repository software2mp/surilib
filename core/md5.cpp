...
/**
 * MD5File: Performs the MD5 algorithm on the first maxBytesToRead bytes of a
 * file (binar or text),returning the results as a char*.
 * Returns NULL if it fails.
 * @param[in] szFilename
 * @param[in] maxBytesToRead
 */
std::string MD5File(const char* szFilename, int maxBytesToRead) {
   ...
   try {
      ...
      if ((file = fopen(szFilename, "rb")) != NULL) {
         while ((nLen = fread(chBuffer, 1, 1024, file)) && (nLenAcum < maxBytesToRead)) {
            nLenAcum += nLen;
            alg.Update(chBuffer, nLen);
         }
